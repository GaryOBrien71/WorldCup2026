...
except Exception:  # pragma: no cover - app can still run in local JSON fallback mode
    create_client = None
...
STATE_PATH = BASE_DIR / "worldcup2026_family_state.json"
DB_TABLE = "app_state"
...
def secret_get(name: str, default=None):
    """Read a Streamlit secret safely in local/dev and cloud environments."""
    try:
...
def require_family_password() -> None:
    """Optional lightweight password gate.

    Add APP_PASSWORD to Streamlit Cloud secrets to enable it.
    If APP_PASSWORD is missing, the app remains open to anyone with the link.
    """
    password = secret_get("APP_PASSWORD")
    if not password:
        return
    if st.session_state.get("auth_ok"):
        return
    st.markdown("## 🔐 World Cup 2026 Family Predictor")
...
def get_supabase_client():
    """Create a Supabase client when cloud database secrets are configured."""
...
def load_persistent_state() -> dict:
    """Load saved family predictions/results from Supabase if configured.

    Supabase is the durable store. The local JSON file is only a fallback for
    local testing or if database secrets are not configured yet.
    """
    client = get_supabase_client()
    state_id = secret_get("STATE_ID", "family")
    if client is not None:
        try:
            response = client.table(DB_TABLE).select("state").eq("id", state_id).execute()
            if response.data:
                return response.data[0].get("state") or {}
            return {}
        except Exception as exc:
            st.sidebar.warning(f"Database load failed; using local fallback. {exc}")
...
def save_persistent_state() -> None:
    """Auto-save the current app state to Supabase, with local JSON fallback."""
    state = {
        "results": st.session_state.get("results", {}),
        "predictions": st.session_state.get("predictions", {}),
        "players": st.session_state.get("players", default_players[:]),
    ...
    if client is not None:
        try:
            ...
            st.sidebar.warning(f"Database autosave failed; using local fallback. {exc}")
    ...
    try:
        ...
        st.sidebar.warning(f"Autosave failed: {exc}")


def init_state() -> None:
    saved = load_persistent_state()
    saved_players = saved.get("players") or default_players[:]
    st.session_state.setdefault("results", saved.get("results", {}))
    st.session_state.setdefault("players", saved_players)
    st.session_state.setdefault("predictions", saved.get("predictions", {p: {} for p in saved_players}))
    st.session_state.setdefault("show_online_flags", False)
    st.session_state.setdefault("knockout", saved.get("knockout", {}))

    # Make sure every current player has a prediction dictionary.
    ...
    with st.sidebar.expander("Players", expanded=False):
        ...
        if st.button("Update players"):
            ...
            if players:
                ...
                st.success("Players updated and auto-saved.")
    with st.sidebar.expander("Save / load state", expanded=False):
        ...
        if upload is not None:
            try:
                ...
                save_persistent_state()
                st.success("State restored and auto-saved. Change page or rerun if needed.")
            except Exception as exc:
        ...
        if st.button("Reset all scores and predictions"):
            ...
            save_persistent_state()
            st.success("Reset complete and auto-saved.")
    db_mode = "Supabase database" if get_supabase_client() is not None else "local JSON fallback"
    st.sidebar.caption(f"Autosave is on. Storage mode: {db_mode}.")
...
def result_editor(page_title: str, data: pd.DataFrame) -> None:
    ...
    if st.button("Save shown scores", type="primary"):
        ...
        save_persistent_state()
        st.success("Scores saved and auto-saved.")
...
def render_predictions() -> None:
    ...
    if st.button("Save shown predictions", type="primary"):
        ...
        save_persistent_state()
        st.success(f"Predictions saved and auto-saved for {player}.")
